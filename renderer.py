...
from constants import (
    COLOR_PLAYER, COLOR_ENEMY, COLOR_WALL, COLOR_FLOOR, 
    COLOR_BULLET, COLOR_BLOOD, MAP_WIDTH, MAP_HEIGHT,
    BULLET_ANIMATION_DELAY, IMPACT_ANIMATION_DELAY, HIT_ANIMATION_DELAY,
    SCREEN_WIDTH, SCREEN_HEIGHT, COLOR_TREE, COLOR_WATER, 
    COLOR_DIRT, COLOR_ROCK, COLOR_CACTUS, COLOR_BUILDING
)
...
class Renderer:
    ...
    def render_game(self, game_state, **kwargs):
        ...
        if kwargs.get('show_impact'):
            self._render_impact(
                kwargs['impact_x'], kwargs['impact_y'], 
    ...
    def _render_bonus_selection(self):
        ...
        self.console.print(
            (SCREEN_WIDTH - len(title)) // 2, 8, 
            title, (255, 255, 0)
        )
        
        subtitle = "Select your character's special ability:"
        self.console.print(
            (SCREEN_WIDTH - len(subtitle)) // 2, 10,
        ...
        for i, bonus in enumerate(bonuses, 1):
            ...
            self.console.print(
                5, start_y + (i-1) * 4,
                bonus_line, (173, 216, 230)
            )
            
            # Description
            self.console.print(
                8, start_y + (i-1) * 4 + 1,
                bonus['description'], (211, 211, 211)
            )
            ...
            self.console.print(
                8, start_y + (i-1) * 4 + 2,
                effects_text, (144, 238, 144)
            )
        
        # Instructions
        instructions = [
            "",
            "Press 1-6 to select your bonus",
            "Press ESC to quit"
        ]
        
        for i, instruction in enumerate(instructions):
            color = (255, 255, 0) if instruction.startswith("Press") else (255, 255, 255)
            self.console.print(
    ...
    def _render_ui(self, game_state):
        ...
        if player_weapon:
            ...
            actual_range = bonus_manager.modify_weapon_range(base_range) if bonus_manager else base_range
        ...
        if message:
            ...
            message_color = (255, 100, 100) if "CRITICAL HIT" in message else (255, 255, 0)  # light red or yellow
    ...
    def animate_bullet(self, start_x, start_y, end_x, end_y, game_map, game_state, hit_target=False):
