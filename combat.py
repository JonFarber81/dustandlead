...
from constants import (
    ...
    DAMAGE_MIN, DAMAGE_MAX, MAP_WIDTH, MAP_HEIGHT
)
...
class CombatSystem:
    ...
    @staticmethod
    def calculate_accuracy(distance, max_range=MAX_SHOOTING_RANGE, min_acc=MIN_ACCURACY):
    ...
    @classmethod
    def attempt_shot_with_weapon_and_bonus(cls, shooter, target, game_map, weapon_stats, bonus_manager=None):
        """
        ...
        base_accuracy = max(min_accuracy, 1.0 - (distance / max_range) * (1.0 - min_accuracy))
        ...
        """
        Attempt a shot from shooter to target using specific weapon stats
        ...
        # Calculate if shot hits using weapon accuracy
        accuracy = max(min_accuracy, 1.0 - (distance / max_range) * (1.0 - min_accuracy))
        hit_roll = random.random() < accuracy
        ...
        if hit_roll and not target_blocked:
            ...
            message = f"{shooter.name}'s {weapon_stats['name']} hits {target.name} for {damage} damage!"
            if target_died:
                message += f" {target.name} is dead!"
            ...
            # Would have hit but blocked by wall
            return {
                'hit': False,
                'bullet_path': bullet_path,
                'bullet_end': (bullet_end_x, bullet_end_y),
    ...
    def attempt_shot(cls, shooter, target, game_map):
        ...
        return cls.attempt_shot_with_weapon(shooter, target, game_map, default_weapon)
